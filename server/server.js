...
// Fallback to fetch API for cloud environments that block Puppeteer
async function fetchProductDataFallback(url) {
  console.log("Using fallback API method for URL:", url);
  
  try {
    // Return basic placeholder data when scraping isn't possible
    return {
      title: "Product information unavailable (Cloud environment limitation)",
      price: "Please try running the app locally for full scraping functionality",
      images: ["https://placehold.co/600x400?text=Scraping+Limited+In+Cloud"],
      description: "Web scraping capabilities are limited on free cloud hosting platforms due to security restrictions. For full functionality, please run the application locally or upgrade to a paid plan that supports browser automation.",
      features: [
        "Limited functionality in cloud deployment",
        "Full scraping works when run locally",
        "Consider using a dedicated scraping API instead"
      ]
    };
  } catch (error) {
...
app.post("/scrape", async (req, res) => {
  ...
  try {
    console.log("Starting scraping process for URL:", link);
    ...
    try {
      ...
      // If puppeteer fails in cloud environment, use fallback
      if (process.env.NODE_ENV === 'production') {
        const fallbackData = await fetchProductDataFallback(link);
        return res.json({
          ...fallbackData,
          _note: "Limited functionality in cloud environment. Full scraping works locally."
        });
      } else {
        // If in dev and puppeteer fails, just throw the error
